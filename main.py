...
#  ffmpeg interaction
def quality(q, urls):
    """Wrapper for choosing a quality."""
    k = sorted(urls.keys(), key=(lambda a: int(a[:-1])))  # sort quality as numbers
...
def slicer(*rules):
    """Return ranges corresponding to chosen episodes from input format."""
    p1 = re.compile(r'(\d+)-(\d+)')
    for i in rules:
        print(i)
        tr = re.search(p1, i)
        if tr:
            ...
            yield range(*(lambda a: (a[0] - 1, a[1], (-1, 1)[a[1] >= a[0]]))(list(map(int, tr.groups()))))
        else:
...
def episode_list_interact():
    rules = filter(''.__ne__, input('> ').split())  # user input and remove empty strings
    try:
        slicer(*rules)
    except LookupError:  # specific for wrong input
        print("Wrong range.")
        return episode_list_interact()  # recursive for wrong input
        # if it fails many times it may crash with a recursion exception
    return rules


def fix_input(inpt, l):
    inpt = inpt[:]
    for i in range(len(inpt)):
        if 'e' in inpt[i]:
            _ = inpt[i].split('-')
            __ = [list(map(int, re.findall(r'\d+', _[k]))) for k in range(2)]
            for ii in range(len(l)):
                if type(__[0]) != int and __[0][0] == l[ii].season and __[0][1] == l[ii].episode:
                    __[0] = ii
                if type(__[1]) != int and __[1][0] == l[ii].season and __[1][1] == l[ii].episode:
                    __[1] = ii
            inpt[i] = '-'.join(map(str, __))
    return inpt
...
if __name__ == '__main__':
    ...
    parser.add_argument('-ep', action='append', type=str, default=None,
                        help='''Episodes to download from page,\
 required to download a page (you will be prompted otherwise)
  Can be specified many times e.g. -ep 12-5 -ep 2; -ep 5-12
  Number is e.g. if season 1 is 20ep, season 2 episode 1 will be number 21.
  or you can specify for example: -ep s1e1-s2e5''')
    parser.add_argument('-a', '--all', action='store_true', help='Download all episodes.')
    ...
    parser.add_argument('--progress', action='store_true', help='Show download progress in download mode.')

    parser.add_argument('-o', '--out', type=str, dest='outfile', default=None,
                        help='''Output file (directory for Page mode) path. Does not overwrite. (video is MP4)
  Default: current directory, guessed file name.''')
    parser.add_argument('--pattern', type=str, default='{name}_s{season}_e{episode}.mp4',
                        help='''File naming pattern. Keywords are {name} - anime name; {season} - season;
  {episode} - episode. Example: {name}_s{season}_e{episode}.mp4 (default) ''')

    parser.add_argument('--chunk-size', type=int, default=2 ** 18,
                        help='''Size of each chunk to download before write.
  May affect download performance, default 262144 (256KB).
  Works best with a power of two.''')
    ...
    try:
        jutsu.get_info(args.url)
        m = 1
    except ValueError:
        try:
            episode_list = jutsu.get_eps(args.url)
            m = 2
        except (ValueError, jutsu.HTTPError):
            try:
    ...
    if m == 1:
        ...
        if args.dmode == 2:
            ...
            outfile = args.outfile if args.outfile else \
                (file_pattern.format(**episode.info)
                 if episode.info else ValueError("Can't guess filename from this link."))
        ...
        if args.ep:
            ...
            print('\n'.join(['{}) S{} EP{}'.format(i + 1,
                                                   *list(episode_list[i].info.values())[1:]) for i
                             in range(len(episode_list))]) +
                  '\nWhat do you want downloaded? (for example 1-15; 1-15 20-25 17; 2)')
            inp = episode_list_interact()
            # interact with user to get required episode list
            # formatting :)
        inp = fix_input(list(inp), episode_list)
        sliced = slicer(*inp)
        epl = [episode_list[j] for j in OrderedDict.fromkeys([ii for i in sliced for ii in i])]
        # make list of eps to download
        # dict duplicate removing is slow but preserves order
        # and my time writing a c wrapper or something :)
        for episode in epl:
            episode.load()
            video = quality(args.quality, episode.urls)
            if args.dmode == 2:
                print(video)
                continue
            outfile = os.path.join(args.outfile, file_pattern.format(**episode.info)) if args.outfile else \
                file_pattern.format(**episode.info)
            jutsu.download(video, outfile, progress=args.progress, chunk_size=args.chunk_size)
